...
void SpotifyRequest::ArtistsReplyReceived(QNetworkReply *reply, const int limit_requested, const int offset_requested) {
  ...
  if (!json_obj.contains(QStringLiteral("artists")) || !json_obj[QStringLiteral("artists")].isObject()) {
    Error(QStringLiteral("Json object missing values."), json_obj);
    ArtistsFinishCheck();
    return;
  }
  QJsonObject obj_artists = json_obj[QStringLiteral("artists")].toObject();
  ...
  int offset = 0;
  if (obj_artists.contains(QStringLiteral("offset"))) {
    offset = obj_artists[QStringLiteral("offset")].toInt();
  }
  ...
  for (const QJsonValueRef value_item : array_items) {
    ...
    QString artist_id = obj_item[QStringLiteral("id")].toString();
    QString artist = obj_item[QStringLiteral("name")].toString();
...
void SpotifyRequest::AlbumsReceived(QNetworkReply *reply, const Artist &artist_artist, const int limit_requested, const int offset_requested) {
  ...
  if (json_obj.contains(QStringLiteral("albums")) && json_obj[QStringLiteral("albums")].isObject()) {
    json_obj = json_obj[QStringLiteral("albums")].toObject();
  }

  if (json_obj.contains(QStringLiteral("tracks")) && json_obj[QStringLiteral("tracks")].isObject()) {
    json_obj = json_obj[QStringLiteral("tracks")].toObject();
  }

  if (!json_obj.contains(QStringLiteral("limit")) ||
      !json_obj.contains(QStringLiteral("offset")) ||
      !json_obj.contains(QStringLiteral("total")) ||
  ...
  int albums_total = json_obj[QStringLiteral("total")].toInt();
  ...
  for (const QJsonValueRef value_item : array_items) {
    ...
    if (obj_item.contains(QStringLiteral("album"))) {
      QJsonValue json_item = obj_item[QStringLiteral("album")];
      if (!json_item.isObject()) {
    ...
    album.album = obj_item[QStringLiteral("name")].toString();

    if (artist_artist.artist_id.isEmpty() && obj_item.contains(QStringLiteral("artists")) && obj_item[QStringLiteral("artists")].isArray()) {
      QJsonArray array_artists = obj_item[QStringLiteral("artists")].toArray();
      for (const QJsonValueRef value : array_artists) {
        if (!value.isObject()) {
          continue;
        }
        QJsonObject obj_artist = value.toObject();
        if (obj_artist.isEmpty() || !obj_artist.contains(QStringLiteral("id")) || !obj_artist.contains(QStringLiteral("name"))) continue;
        artist.artist_id = obj_artist[QStringLiteral("id")].toString();
        artist.artist = obj_artist[QStringLiteral("name")].toString();
        break;
      }
    }
    else {
    ...
    if (obj_item.contains(QStringLiteral("images")) && obj_item[QStringLiteral("images")].isArray()) {
      QJsonArray array_images = obj_item[QStringLiteral("images")].toArray();
      for (const QJsonValueRef value : array_images) {
        if (!value.isObject()) {
          continue;
        }
        QJsonObject obj_image = value.toObject();
        if (obj_image.isEmpty() || !obj_image.contains(QStringLiteral("url")) || !obj_image.contains(QStringLiteral("width")) || !obj_image.contains(QStringLiteral("height"))) continue;
        int width = obj_image[QStringLiteral("width")].toInt();
        int height = obj_image[QStringLiteral("height")].toInt();
        if (width <= 300 || height <= 300) {
          continue;
        }
        album.cover_url = QUrl(obj_image[QStringLiteral("url")].toString());
      }
    }

    if (obj_item.contains(QStringLiteral("tracks")) && obj_item[QStringLiteral("tracks")].isObject()) {
      QJsonObject obj_tracks = obj_item[QStringLiteral("tracks")].toObject();
      if (obj_tracks.contains(QStringLiteral("items")) && obj_tracks[QStringLiteral("items")].isArray()) {
        QJsonArray array_tracks = obj_tracks[QStringLiteral("items")].toArray();
...
void SpotifyRequest::SongsReceived(QNetworkReply *reply, const Artist &artist, const Album &album, const int limit_requested, const int offset_requested) {
  ...
  int offset = json_obj[QStringLiteral("offset")].toInt();
  int songs_total = json_obj[QStringLiteral("total")].toInt();
  ...
  for (const QJsonValueRef value_item : array_items) {
    ...
    if (obj_item.contains(QStringLiteral("item")) && obj_item[QStringLiteral("item")].isObject()) {
      obj_item = obj_item[QStringLiteral("item")].toObject();
    }

    if (obj_item.contains(QStringLiteral("track")) && obj_item[QStringLiteral("track")].isObject()) {
      obj_item = obj_item[QStringLiteral("track")].toObject();
    }
...
void SpotifyRequest::ParseSong(Song &song, const QJsonObject &json_obj, const Artist &album_artist, const Album &album) {
  ...
  if (
      ...
      !json_obj.contains(QStringLiteral("name")) ||
      !json_obj.contains(QStringLiteral("uri")) ||
      !json_obj.contains(QStringLiteral("duration_ms")) ||
      !json_obj.contains(QStringLiteral("track_number")) ||
      !json_obj.contains(QStringLiteral("disc_number"))
    ) {
  ...
  if (json_obj.contains(QStringLiteral("artists")) && json_obj[QStringLiteral("artists")].isArray()) {
    QJsonArray array_artists = json_obj[QStringLiteral("artists")].toArray();
    for (const QJsonValueRef value_artist : array_artists) {
      if (!value_artist.isObject()) continue;
       QJsonObject obj_artist = value_artist.toObject();
       if (!obj_artist.contains(QStringLiteral("type")) || !obj_artist.contains(QStringLiteral("id")) || !obj_artist.contains(QStringLiteral("name"))) {
         continue;
       }
       artist_id = obj_artist[QStringLiteral("id")].toString();
       artist_title = obj_artist[QStringLiteral("name")].toString();
       break;
    }
  }
  ...
  QString album_title;
  QUrl cover_url;
  if (json_obj.contains(QStringLiteral("album")) && json_obj[QStringLiteral("album")].isObject()) {
    QJsonObject obj_album = json_obj[QStringLiteral("album")].toObject();
    if (obj_album.contains(QStringLiteral("type")) && obj_album.contains(QStringLiteral("id")) && obj_album.contains(QStringLiteral("name"))) {
      album_id = obj_album[QStringLiteral("id")].toString();
      album_title = obj_album[QStringLiteral("name")].toString();
      if (obj_album.contains(QStringLiteral("images")) && obj_album[QStringLiteral("images")].isArray()) {
        ...
        for (const QJsonValueRef value : array_images) {
          ...
          cover_url = QUrl(obj_image[QStringLiteral("url")].toString());
        }
      }
    }
  }
  ...
  QString song_id = json_obj[QStringLiteral("id")].toString();
  QString title = json_obj[QStringLiteral("name")].toString();
  QString uri = json_obj[QStringLiteral("uri")].toString();
  qint64 duration = json_obj[QStringLiteral("duration_ms")].toVariant().toLongLong() * kNsecPerMsec;
  int track = json_obj[QStringLiteral("track_number")].toInt();
  int disc = json_obj[QStringLiteral("disc_number")].toInt();
