    ...
    if (reply->error() != QNetworkReply::NoError && reply->error() < 200) {
      ...
      if (json_error.error == QJsonParseError::NoError && !json_doc.isEmpty() && json_doc.isObject()) {
        ...
        if (!json_obj.isEmpty() && json_obj.contains(QStringLiteral("error")) && json_obj[QStringLiteral("error")].isObject()) {
          ...
          if (!obj_error.isEmpty() && obj_error.contains(QStringLiteral("status")) && obj_error.contains(QStringLiteral("message"))) {
            status = obj_error[QStringLiteral("status")].toInt();
            QString user_message = obj_error[QStringLiteral("message")].toString();
