...
constexpr Song::Source kSource = Song::Source::Spotify;
...
constexpr char kArtistsSongsFtsTable[] = "spotify_artists_songs_fts";
constexpr char kAlbumsSongsFtsTable[] = "spotify_albums_songs_fts";
constexpr char kSongsFtsTable[] = "spotify_songs_fts";
...
SpotifyService::SpotifyService(Application *app, QObject *parent)
    : InternetService(Song::Source::Spotify, QStringLiteral("Spotify"), QStringLiteral("spotify"), QLatin1String(SpotifySettingsPage::kSettingsGroup), SettingsDialog::Page::Spotify, app, parent),
      app_(app),
  ...
  artists_collection_backend_->Init(app_->database(), app->task_manager(), Song::Source::Spotify, QLatin1String(kArtistsSongsTable), QLatin1String(kArtistsSongsFtsTable));

  albums_collection_backend_ = make_shared<CollectionBackend>();
  albums_collection_backend_->moveToThread(app_->database()->thread());
  albums_collection_backend_->Init(app_->database(), app->task_manager(), Song::Source::Spotify, QLatin1String(kAlbumsSongsTable), QLatin1String(kAlbumsSongsFtsTable));

  songs_collection_backend_ = make_shared<CollectionBackend>();
  songs_collection_backend_->moveToThread(app_->database()->thread());
  songs_collection_backend_->Init(app_->database(), app->task_manager(), Song::Source::Spotify, QLatin1String(kSongsTable), QLatin1String(kSongsFtsTable));
