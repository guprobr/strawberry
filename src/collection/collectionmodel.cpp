...
void CollectionModel::ReloadSettings() {
  ...
  if (show_pretty_covers != show_pretty_covers_ ||
      show_dividers != show_dividers_ ||
      show_various_artists != show_various_artists_ ||
      sort_skips_articles != sort_skips_articles_) {
    show_pretty_covers_ = show_pretty_covers;
...
void CollectionModel::SetFilterMode(const CollectionFilterOptions::FilterMode filter_mode) {
  ...
  if (filter_options_.filter_mode() != filter_mode) {
...
void CollectionModel::SetFilterMaxAge(const int filter_max_age) {
  ...
  if (filter_options_.max_age() != filter_max_age) {
    filter_options_.set_max_age(filter_max_age);
    ScheduleReset();
  }
...
void CollectionModel::AddReAddOrUpdateSongsInternal(const SongList &songs) {
  ...
  for (const Song &new_song : songs) {
    ...
    for (int i = 0; i < 3; ++i) {
      if (IsArtistGroupBy(group_by_[i]) && new_song.is_compilation() != old_song.is_compilation()) {
        container_key_changed = true;
      }
      if (group_by_[i] != GroupBy::None && ContainerKey(group_by_[i], separate_albums_by_grouping_, new_song) != ContainerKey(group_by_[i], separate_albums_by_grouping_, old_song)) {
        container_key_changed = true;
      }
    }
...
void CollectionModel::AddSongsInternal(const SongList &songs) {
  ...
  for (const Song &song : songs) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (IsArtistGroupBy(group_by) && song.is_compilation() && show_various_artists_) {
        ...
        container_key.append(ContainerKey(group_by, separate_albums_by_grouping_, song));
        ...
        if (container_nodes_[i].contains(container_key)) {
          ...
          container = CreateItem(group_by, separate_albums_by_grouping_, true, i == 0, container, song, i);
          container_nodes_[i].insert(container_key, container);
        }
    ...
    song_nodes_.insert(song.id(), CreateItem(GroupBy::None, separate_albums_by_grouping_, true, false, container, song, -1));
  }
...
void CollectionModel::UpdateSongsInternal(const SongList &songs) {
  ...
  for (const Song &new_song : songs) {
    if (!song_nodes_.contains(new_song.id())) {
      qLog(Error) << "Song does not exist in model" << new_song.id() << new_song.effective_albumartist() << new_song.effective_album() << new_song.title();
      continue;
    }
    ...
    SetItemData(item, new_song);
    if (art_changed) {
      for (CollectionItem *parent = item->parent; parent != root_; parent = parent->parent) {
        if (IsAlbumGroupBy(group_by_[parent->container_level])) {
...
CollectionItem *CollectionModel::CreateItem(const GroupBy group_by, const bool separate_albums_by_grouping, const bool signal, const bool create_divider, CollectionItem *parent, const Song &s, const int container_level) {

  CollectionItem *item = InitItem(group_by, signal, parent, container_level);

  if (parent != root_ && !parent->container_key.isEmpty()) {
    item->container_key = parent->container_key + QLatin1Char('-');
  }

  switch (group_by) {
    case GroupBy::AlbumArtist:{
      item->metadata.set_albumartist(s.effective_albumartist());
      item->container_key.append(ContainerKey(group_by, separate_albums_by_grouping, s));
...
void CollectionModel::SetItemData(CollectionItem *item, const Song &song) {

  item->metadata = song;

  if (item->parent == root_) {
    item->container_key = TextOrUnknown(song.title());
  }
  else {
    item->container_key = item->parent->container_key + QLatin1Char('-') + TextOrUnknown(song.title());
  }
...
void CollectionModel::RowsInserted(const QModelIndex &parent, const int first, const int last) {
  ...
  for (int i = first; i <= last; i++) {
    ...
    if (!idx.isValid()) continue;
    CollectionItem *item = IndexToItem(idx);
    if (!item || item->type != CollectionItem::Type_Song) continue;
    songs << item->metadata;
  }
